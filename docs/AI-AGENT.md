...
## 9. Battery-driven loop hardening (2026-07-29 · measurement = docs/EVAL.md, rationale = ADR-11)

- **Live progress streaming (`/assistant/message/stream`):** the agent reports plan/step/tool events through an `on_event` observer; the web layer relays them as SSE so plan steps and tool calls appear the moment they run. Observers are best-effort — a broken callback can never break a turn.
