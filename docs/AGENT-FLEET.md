...
- **001은 팔려고 만드는 제품.** 특정 회사가 아니라 **전형적 소규모 스타트업(1~30명)** 을 모델로 설계.
  *(a previous company QBO 데이터는 개발·테스트용일 뿐, 타겟 고객 프로필이 아님.)*
...
### 활성화 모델 — 전부 항상 가용 (잠자기·확인 없음)
- **모든 롤은 항상 듣고 있다.** 평소 안 쓰는 롤도 켜둠 — 노는 비용이 ~0이기 때문:
  ```
  루프가 깸 → 내 큐에 일 있나? (DB 조회, GPU 0) → 없으면 즉시 잠
  일 있을 때만 → LLM 호출 (GPU 사용)
  ```
  즉 **"루프가 돈다 ≠ GPU를 쓴다."** GPU는 실제 처리할 일이 있을 때만.
- **자동 활성화 + 통보(논블로킹):** 새 유형의 일이 오면 멋대로 켜지되, 창업자에겐 "켰어요"
  **통보만** (승인 안 물음). 관련 없는 롤(SaaS의 재고)은 일이 안 와서 그냥 영원히 조용.
- **예외:** 특정 롤을 일부러 안 쓰려면(예: 급여는 Gusto로 외부 처리) 설정에서 **"영구 끔"**.
- **진짜 안전장치는 하류:** 자동으로 켜져도 드래프트·기표·지급은 전부 창업자 승인 게이트.
...
**결정 완료 (D6):**
- [x] 타겟 = 제네릭 소규모 스타트업 (판매 제품), a previous company는 테스트 데이터
- [x] 롤 = 코어 5 + 선택 4, **전부 항상 가용**, 자동 활성화 + 통보, 영구끔 옵션
...
**남은 질문:**
- [ ] 💰매출·수금 롤의 트리거 입력 (고객주문은 어디서? 수동입력/이메일/연동?)
- [ ] 📊자금·인사이트가 보여줄 핵심 지표 셋 (현금·번레이트·런웨이 + α?)
- [ ] 선택 롤 자동 활성화의 "통보" UI 형식
