...
def sales_pipeline(request: Request, user=Depends(require_login),
...
def send_quote(quote_id: int, user=Depends(require_login),
               session: Session = Depends(get_session)):
...
def invoice_order(so_id: int, user=Depends(require_login),
                  session: Session = Depends(get_session)):
    if _can_finance(user):
